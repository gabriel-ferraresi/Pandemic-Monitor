...
export function Sidebar({ data, onAlertClick, activeGlobeFilter = 'all', onGlobeFilterChange }: SidebarProps) {
  ...
  return (
    <div className="w-[400px] h-full flex flex-col gap-6 p-6 overflow-y-auto bg-white/60 dark:bg-black/60 backdrop-blur-xl border-l border-slate-200 dark:border-white/10 z-10 relative shadow-[4px_0_24px_rgba(0,0,0,0.05)] dark:shadow-[-4px_0_24px_rgba(0,0,0,0.5)] transition-colors duration-500 custom-scrollbar">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        {/* Nível de Ameaça Global — substitui Registros Forenses */}
        <div className={cn(
          ...
          <div className="flex items-center justify-between mb-3 relative z-10">
            ...
            <span className="text-[10px] font-mono text-slate-600 dark:text-zinc-500 bg-white dark:bg-black/50 px-2 py-0.5 rounded-full border border-slate-200 dark:border-white/5 transition-colors">Nível Global</span>
          </div>
          <div className={cn("text-lg font-black tracking-tight relative z-10 transition-colors", threatStyle.text)}>
            {threatLevel}
          </div>
          <div className="text-[10px] text-slate-600 dark:text-zinc-400 font-medium uppercase tracking-wider mt-1 relative z-10 transition-colors">
            Ameaça Global
          </div>
        </div>

        {/* Patógenos Monitorados — clicável para filtrar globo */}
        <div
          onClick={() => handleFilterClick('outbreaks')}
          className={cn(
            "p-4 rounded-xl bg-gradient-to-br from-slate-50 to-white dark:from-white/5 dark:to-transparent border transition-all cursor-pointer group relative overflow-hidden shadow-[0_0_15px_rgba(59,130,246,0.15)]",
        ...
        >
          ...
          <div className="flex items-center justify-between mb-3 relative z-10">
            ...
            <span className="text-[10px] font-mono text-slate-600 dark:text-zinc-500 group-hover:text-slate-800 dark:group-hover:text-zinc-400 bg-white dark:bg-black/50 px-2 py-0.5 rounded-full border border-slate-200 dark:border-white/5 transition-colors">Em tempo real</span>
          </div>
          <div className="text-2xl font-bold text-slate-800 dark:text-white tracking-tight relative z-10 transition-colors">
            <AnimatedCounter value={data.stats.monitoredPathogens} />
          </div>
          <div className="text-[10px] text-slate-600 dark:text-zinc-400 font-medium uppercase tracking-wider mt-1 relative z-10 transition-colors">Patógenos Monitorados</div>
        </div>

        {/* Anomalias Ativas — clicável para filtrar globo */}
        <div
          onClick={() => handleFilterClick('anomalies')}
          className={cn(
            "p-4 rounded-xl bg-gradient-to-br from-slate-50 to-white dark:from-white/5 dark:to-transparent border transition-all cursor-pointer group relative overflow-hidden shadow-[0_0_15px_rgba(249,115,22,0.15)]",
        ...
        >
          ...
          <div className="flex items-center justify-between mb-3 relative z-10">
            ...
            <span className="text-[10px] font-mono text-slate-600 dark:text-zinc-500 group-hover:text-slate-800 dark:group-hover:text-zinc-400 bg-white dark:bg-black/50 px-2 py-0.5 rounded-full border border-slate-200 dark:border-white/5 transition-colors">Investigando</span>
          </div>
          <div className="text-2xl font-bold text-slate-800 dark:text-white tracking-tight relative z-10 transition-colors">
            <AnimatedCounter value={data.stats.activeAnomalies} />
          </div>
          <div className="text-[10px] text-slate-600 dark:text-zinc-400 font-medium uppercase tracking-wider mt-1 relative z-10 transition-colors">Anomalias Ativas</div>
        </div>

        {/* Previsões Geradas */}
        <div className="p-4 rounded-xl bg-gradient-to-br from-slate-50 to-white dark:from-white/5 dark:to-transparent border border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 transition-all cursor-default group relative overflow-hidden shadow-[0_0_15px_rgba(16,185,129,0.15)]">
          <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/50 dark:via-white/5 to-transparent -translate-x-full group-hover:animate-shimmer" />
          <div className="flex items-center justify-between mb-3 relative z-10">
            <BrainCircuit className="w-4 h-4 text-emerald-600 dark:text-emerald-500" />
            <span className="text-[10px] font-mono text-slate-600 dark:text-zinc-500 group-hover:text-slate-800 dark:group-hover:text-zinc-400 bg-white dark:bg-black/50 px-2 py-0.5 rounded-full border border-slate-200 dark:border-white/5 transition-colors">Próx. 30 dias</span>
          </div>
          <div className="text-2xl font-bold text-slate-800 dark:text-white tracking-tight relative z-10 transition-colors">
            <AnimatedCounter value={data.predictions?.length || 0} />
          </div>
          <div className="text-[10px] text-slate-600 dark:text-zinc-400 font-medium uppercase tracking-wider mt-1 relative z-10 transition-colors">Previsões Geradas</div>
        </div>
      </div>
      ...
      <div className="flex flex-col mb-2">
        ...
        <div className="flex flex-col gap-3">
          {data.predictions.map((pred, index) => (
            <div key={`${pred.id}-${index}`} className="p-4 rounded-xl bg-gradient-to-r from-emerald-50 to-white dark:from-emerald-500/10 dark:to-transparent border border-emerald-200 dark:border-emerald-500/30 transition-colors relative">
        ...
        <div className="flex flex-col gap-3 pr-2 overflow-y-auto custom-scrollbar">
          {data.outbreaks.map((outbreak, index) => (
            <div
              ...
              className={cn(
                "p-4 rounded-xl border transition-all cursor-pointer hover:translate-x-1 relative",
