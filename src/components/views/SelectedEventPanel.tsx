...
export function SelectedEventPanel({ event, onClose, isMobile = false }: { event: any, onClose: () => void, isMobile?: boolean }) {
  if (!event) return null;

  return (
    <div className={cn(
      "bg-white/90 dark:bg-black/80 backdrop-blur-2xl border border-slate-200 dark:border-white/10 rounded-2xl shadow-[0_0_40px_rgba(0,0,0,0.1)] dark:shadow-[0_0_40px_rgba(0,0,0,0.8)] overflow-hidden z-40 animate-in fade-in slide-in-from-bottom-8 duration-500 transition-colors",
      isMobile
        ? "fixed bottom-[136px] left-3 right-3 max-h-[60vh] overflow-y-auto"
        : "absolute bottom-8 left-24 w-[420px]"
    )}>
      {/* Top accent line */}
      ...
      <div className={cn("p-6", isMobile && "p-4")}>
        <div className="flex justify-between items-start mb-4 md:mb-6">
          <div className="flex items-center gap-3">
            <div className="p-2 md:p-2.5 rounded-xl flex items-center justify-center" style={{ backgroundColor: `${event.color}20`, color: event.color }}>
              {event.type === 'outbreak' ? <Activity className="w-5 h-5 md:w-6 md:h-6" /> : <Radar className="w-5 h-5 md:w-6 md:h-6" />}
            </div>
            <div>
              <h2 className="text-lg md:text-xl font-bold text-slate-800 dark:text-white tracking-tight transition-colors">{event.title}</h2>
              <div className="flex items-center gap-1 text-xs md:text-sm text-slate-500 dark:text-zinc-400 mt-0.5 transition-colors">
                <MapPin className="w-3 h-3 md:w-3.5 md:h-3.5" /> {event.location}
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 md:gap-4 mb-4 md:mb-6">
          <div className="p-3 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5 transition-colors">
            ...
            <div className="text-xs md:text-sm font-mono text-slate-700 dark:text-zinc-300 flex items-center gap-2 transition-colors">
              <Crosshair className="w-3 h-3 md:w-3.5 md:h-3.5 text-slate-400 dark:text-zinc-500" />
          ...
          {event.type === 'outbreak' && (
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5 transition-colors">
              ...
              <div className="text-xs md:text-sm font-mono font-bold text-slate-800 dark:text-white transition-colors">{event.cases}</div>
            </div>
          )}
          ...
          {event.type === 'anomaly' && (
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5 transition-colors">
              ...
              <div className="text-xs md:text-sm font-mono font-bold text-slate-800 dark:text-white transition-colors">{event.confidence}%</div>
            </div>
          )}
        </div>

        <div className="p-3 md:p-4 rounded-xl bg-gradient-to-br from-slate-50 to-white dark:from-white/5 dark:to-transparent border border-slate-200 dark:border-white/5 transition-colors">
          ...
          <p className="text-xs md:text-sm text-slate-600 dark:text-zinc-300 leading-relaxed transition-colors">{event.summary}</p>
        </div>
